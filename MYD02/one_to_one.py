...
tatm_rows_to_del = [] #rows where average row diffence is NOT in [-1, 1]

for row, x in enumerate(tatm_2km):
    df = np.subtract(x, tatm_ig_2km[row])
#    print df
    avg = np.average(df)
    if avg >=-1 and avg <=1:
        pass
    else:
        tatm_rows_to_del.append(row)
        

#==============================================================================
# Get index rows where dif row average is [-1, 1]
#==============================================================================
#for cluster_count in np.arange(2, 8):
    
tatm_2km = np.delete(tatm_2km, tatm_rows_to_del, axis =0)
#tatm_ig_2km = np.delete(tatm_ig_2km, tatm_rows_to_del, axis =0)
...
cluster_path = '/Users/sevakm/Documents/AIRS_To_Modis/one_to_neg_one/one_to_one.np.npy'
np.save(cluster_path, np.array(current_tatm_delete_indices))

more_del = np.load(cluster_path)

tatm_2km = np.delete(tatm_2km, more_del, axis =0) #tatm rows in range [-1, 1]


one_to_one_indices =[]#indices of clusters where rows dif avg in range [-1, 1]
...
print np.array(cluster_list).shape

for index ,x in enumerate(range(tatm_2km.shape[0])):
    if x not in one_to_one_indices:
        tatm_pos_arr= []
        ...
        if tatm_pos_arr in cluster_list:
            one_to_one_indices.append(cluster_list.index(tatm_pos_arr))
        
        print str(index) + ": " + str(tatm_2km.shape[0])

cluster_path = '/Users/sevakm/Documents/AIRS_To_Modis/one_to_neg_one/one_to_one_index.npy'
np.save(cluster_path, np.array(one_to_one_indices))
#r, c = tatm_2km.shape
#dif = np.zeros([r, c])
#dif[dif ==0] = 9999
#
...
#
#plt.figure()
#n, b ,p =plt.hist(dif.compressed(), normed=1, facecolor='green', 
#                  align ='mid', range = [-1, 1], bins =1000)
#plt.grid(True)
##
#plt.title(
##        "STD range: [" + str(round(range_four, 2)) +  ", "  
