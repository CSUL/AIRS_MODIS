...
#FILE_NAME1 = '/Users/sevakm/Documents/AIRS_To_Modis/MYD03/air/AIRS.2013.03.02.150.L1B.AIRS_Rad.v5.0.21.0.G13063174326.hdf'
...
data11 = hdf1.select(DATAFIELD_NAME)[:,:]
data22 = hdf2.select(DATAFIELD_NAME)[:,:]
data33 = hdf3.select(DATAFIELD_NAME)[:,:]
data44 = hdf4.select(DATAFIELD_NAME)[:,:]
data55 = hdf5.select(DATAFIELD_NAME)[:,:]
lat_data=np.concatenate( [data11,data22,data33,data44,data55], axis=0)
...
#for x in np.arange(0, 90, 8):
#    print x

string =""
counter = 0
for x in range(675):
 #    for y in np.arange(0, 90, 8):
 #        if lon_data[x][y] <0 or lat_data[x][y] < 0:
 #            pass  
         
 #        else:
     airs_lon = lon_data[x][45]
     airs_lat = lat_data[x][45]
     string = string + str(lon_data[x][45])+ "," + str( lat_data[x][45]) + "\n"
     counter = counter+1
 
 #        print string
 #
text_file = open("/Users/sevakm/Documents/AIRS_To_Modis/MYD03/AIRS_MID_COL.txt", "w") 
